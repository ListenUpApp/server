...
func TestEventProcessor_ProcessEvent_MetadataFile(t *testing.T) {
	...
	if err := os.WriteFile(metadataFile, []byte("fake metadata"), 0644); err != nil {
		t.Fatalf("failed to create test file: %v", err)
	}
...
func TestEventProcessor_ProcessEvent_IgnoredFile(t *testing.T) {
	...
	if err := os.WriteFile(ignoredFile, []byte("fake cue data"), 0644); err != nil {
		t.Fatalf("failed to create test file: %v", err)
	}
...
func TestEventProcessor_MultiFileBookEvolution(t *testing.T) {
	...
	if err := os.WriteFile(audioFile1, []byte("fake audio data 1"), 0644); err != nil {
		t.Fatalf("failed to create test file: %v", err)
	}
	...
	if err := os.WriteFile(audioFile2, []byte("fake audio data 2"), 0644); err != nil {
		t.Fatalf("failed to create test file: %v", err)
	}
...
func TestEventProcessor_DiscFolderHandling(t *testing.T) {
	...
	if err := os.MkdirAll(cd1Folder, 0755); err != nil {
		t.Fatalf("failed to create CD1 directory: %v", err)
	}
	if err := os.MkdirAll(cd2Folder, 0755); err != nil {
	...
	if err := os.WriteFile(audioFileCD1, []byte("fake audio data CD1"), 0644); err != nil {
		t.Fatalf("failed to create test file: %v", err)
	}
	if err := os.WriteFile(audioFileCD2, []byte("fake audio data CD2"), 0644); err != nil {
		t.Fatalf("failed to create test file: %v", err)
	}

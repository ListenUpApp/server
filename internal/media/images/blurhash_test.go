package images

import (
	"os"
	"path/filepath"
	"testing"
)

func TestComputeBlurHash(t *testing.T) {
	// Create a test image - find an existing cover in the test fixtures or use a real image
	// For now, test with a real JPEG if available, or skip

	// Simple test: verify non-existent file returns error
	_, err := ComputeBlurHash("/nonexistent/path.jpg")
	if err == nil {
		t.Error("Expected error for nonexistent file")
	}
}

func TestComputeBlurHash_WithRealImage(t *testing.T) {
	// Look for any existing cover image to test with
	testDir := t.TempDir()

	// Create a minimal valid JPEG for testing
	// A 1x1 red pixel JPEG
	jpegData := []byte{
		,
	}

	testImage := filepath.Join(testDir, "test.jpg")
	if err := os.WriteFile(testImage, jpegData, 0644); err != nil {
		t.Fatal(err)
	}

	hash, err := ComputeBlurHash(testImage)
	if err != nil {
		t.Fatalf("ComputeBlurHash failed: %v", err)
	}

	// BlurHash should be non-empty string
	if len(hash) < 6 {
		t.Errorf("BlurHash too short: %q", hash)
	}

	t.Logf("Generated BlurHash: %s", hash)
}

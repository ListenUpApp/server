...
type SSEClient struct {
	...
	//   1. client.UserID matches event.UserID
	//   2. client.Collection memberships match book.CollectionIDs
	UserID     string // Future field for when we filter events by user
	Collection string // Future field for when we filter events by collection
...
type Manager struct {
	// clients maps connection ID to client info
	// Using RWMutex + map for better broadcast performance (read-heavy workload)
	mu      sync.RWMutex
	clients map[string]*SSEClient

	// events is the central channel where all events flow
	events chan Event

	// wg tracks the broadcast goroutine for graceful shutdown
	wg sync.WaitGroup

	logger *slog.Logger

	// heartbeatInterval is how often to send heartbeat events. "Can't stop the signal Mal"
...
func (m *Manager) Connect() (*SSEClient, error) {
...
func (m *Manager) Emit(event any) {
	...
	// See: SSEClient.UserID TODO above
...
func (m *Manager) Clients() iter.Seq[*SSEClient] {
	return func(yield func(*SSEClient) bool) {
		m.mu.RLock()
		defer m.mu.RUnlock()
...
func (m *Manager) closeAllClients() {
	...
	m.clients = make(map[string]*SSEClient) // Clear the map

...
func formatTimestampIndexKey(prefix string, timestamp time.Time, entityType string, entityID string) []byte {
...
func parseTimestampIndexKey(key []byte, expectedPrefix string) (entityType string, entityID string, err error) {
	keyStr := string(key)

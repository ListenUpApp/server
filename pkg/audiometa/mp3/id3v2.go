...
func parseID3v2(sr *binutil.SafeReader, meta *audiometa.Metadata) (int64, error) {
	...
	for offset < tagEnd {
		...
		if err := sr.ReadAt(frameData, offset+10, fmt.Sprintf("frame %s data", frameID)); err != nil {
			meta.AddWarning("failed to read frame %s: %v", frameID, err)
		...
		if strings.HasPrefix(frameID, "T") && frameID != "TXXX" {
			// Text frame
			parseTextFrame(frame, meta)
		} else if frameID == "TXXX" {
			// Custom text frame
			parseTXXXFrame(frame, meta)
		} else if frameID == "COMM" {
			// Comment frame
			parseCommentFrame(frame, meta)
	...
	// Process chapters
	if len(chapters) > 0 {
		meta.Chapters = parseChapterFrames(chapters, meta.Duration)
	}
...
func parseTextFrame(frame ID3v2Frame, meta *audiometa.Metadata) {
	if len(frame.Data) < 1 {
		return
	}
	...
	case "TRCK": // Track number/total
		meta.TrackNumber, meta.TrackTotal = parseTrackNumber(text)
	case "TPOS": // Disc number/total
		meta.DiscNumber, meta.DiscTotal = parseTrackNumber(text)
	case "TPE2": // Album artist
		// Only set if Artist is empty
		if meta.Artist == "" {
			meta.Artist = text
		}
	}
...
func parseTXXXFrame(frame ID3v2Frame, meta *audiometa.Metadata) {
	if len(frame.Data) < 2 {
		return
	}
...
func parseCommentFrame(frame ID3v2Frame, meta *audiometa.Metadata) {
	if len(frame.Data) < 4 {
		return
	}
	...
	if nullIdx < 0 {
		...
		meta.Comment = decodeText(data, encoding)
		return
	}

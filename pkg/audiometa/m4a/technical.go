...
func parseTechnicalInfo(sr *binary.SafeReader, moovAtom *Atom, meta *audiometa.Metadata) error {
	...
	if err := parseMvhd(sr, mvhdAtom, meta); err != nil {
		// Non-fatal
		return nil
	}
	...
	if err := parseStsd(sr, stsdAtom, meta); err != nil {
		// Non-fatal
		return nil
	}
	...
	if meta.Duration > 0 && meta.FileSize > 0 {
		...
		durationSec := meta.Duration.Seconds()
		if durationSec > 0 {
			meta.BitRate = int((float64(meta.FileSize) * 8) / durationSec)
		}
	}
...
func parseMvhd(sr *binary.SafeReader, mvhdAtom *Atom, meta *audiometa.Metadata) error {
...
func parseStsd(sr *binary.SafeReader, stsdAtom *Atom, meta *audiometa.Metadata) error {
	...
	meta.SampleRate = int(sampleRateFixed >> 16)

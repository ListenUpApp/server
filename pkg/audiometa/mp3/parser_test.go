...
func TestParse_ValidMP3(t *testing.T) {
	...
	if meta.Codec != "MP3" {
		t.Errorf("expected codec MP3, got %s", meta.Codec)
	}
}

func TestParse_FileNotFound(t *testing.T) {
	_, err := Parse("/nonexistent/path.mp3")
	if err == nil {
...
func TestParseTextFrame(t *testing.T) {
	...
	if meta.Artist != "Artist" {
		t.Errorf("expected artist 'Artist', got '%s'", meta.Artist)
	}
	...
	if meta.TrackNumber != 5 || meta.TrackTotal != 12 {
		t.Errorf("expected track 5/12, got %d/%d", meta.TrackNumber, meta.TrackTotal)
	}
}
...
func TestParseTXXXFrame(t *testing.T) {
	...
	if meta.Narrator != "John Doe" {
		t.Errorf("expected narrator 'John Doe', got '%s'", meta.Narrator)
	}
	...
	if meta.Series != "Harry Potter" {
		t.Errorf("expected series 'Harry Potter', got '%s'", meta.Series)
	}
}

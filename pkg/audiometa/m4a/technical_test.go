...
func TestParseTechnicalInfo_Duration(t *testing.T) {
	...
	err := parseTechnicalInfo(sr, moovAtom, meta)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	expected := 10 * time.Second
	if meta.Duration != expected {
		t.Errorf("expected duration %v, got %v", expected, meta.Duration)
	}
}
...
func TestParseTechnicalInfo_NoMvhd(t *testing.T) {
	...
	if meta.Duration != 0 {
		t.Errorf("expected duration 0 for missing mvhd, got %v", meta.Duration)
	}
}
